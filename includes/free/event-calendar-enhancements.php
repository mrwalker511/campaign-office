...
class CP_Event_Calendar_Enhancements {
    ...
    public function add_admin_menu() {
        add_submenu_page(
            ...
            __('Calendar View', 'campaign-office'),
    ...
    public function add_location_meta_box() {
        add_meta_box(
            'cp_event_location',
            __('Event Location', 'campaign-office'),
    ...
    public function render_location_meta_box($post) {
        ...
        <div class="cp-event-location-fields">
            <p>
                <label for="cp_event_venue"><strong><?php esc_html_e('Venue Name:', 'campaign-office'); ?></strong></label><br>
            ...
            <div style="display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 10px;">
                ...
                <p>
                    <label for="cp_event_zip"><strong><?php esc_html_e('ZIP:', 'campaign-office'); ?></strong></label><br>
                    <input type="text" id="cp_event_zip" name="cp_event_zip" value="<?php echo esc_attr($zip); ?>" class="widefat">
                </p>
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                <p>
                    <label for="cp_event_latitude"><strong><?php esc_html_e('Latitude (optional):', 'campaign-office'); ?></strong></label><br>
                    <input type="text" id="cp_event_latitude" name="cp_event_latitude" value="<?php echo esc_attr($lat); ?>" class="widefat">
                </p>
                <p>
                    <label for="cp_event_longitude"><strong><?php esc_html_e('Longitude (optional):', 'campaign-office'); ?></strong></label><br>
                    <input type="text" id="cp_event_longitude" name="cp_event_longitude" value="<?php echo esc_attr($lng); ?>" class="widefat">
                </p>
            </div>
            <p class="description">
                <?php esc_html_e('Coordinates are optional. If provided, a Google Map will be displayed automatically.', 'campaign-office'); ?>
            </p>
        </div>
        <?php
    }
    ...
    public function render_calendar_shortcode($atts) {
        ...
        <div class="cp-event-calendar" data-view="<?php echo esc_attr($atts['view']); ?>">
            <div class="cp-calendar-header">
                <button class="cp-calendar-prev" aria-label="<?php esc_attr_e('Previous', 'campaign-office'); ?>">
                    <span class="dashicons dashicons-arrow-left-alt2"></span>
                </button>
                <h2 class="cp-calendar-title"></h2>
                <button class="cp-calendar-next" aria-label="<?php esc_attr_e('Next', 'campaign-office'); ?>">
                    <span class="dashicons dashicons-arrow-right-alt2"></span>
                </button>
            </div>
            <div class="cp-calendar-view-switcher">
                <button class="cp-view-btn" data-view="month"><?php esc_html_e('Month', 'campaign-office'); ?></button>
                <button class="cp-view-btn" data-view="week"><?php esc_html_e('Week', 'campaign-office'); ?></button>
                <button class="cp-view-btn" data-view="list"><?php esc_html_e('List', 'campaign-office'); ?></button>
            </div>
    ...
    private function render_month_view($year_month, $events) {
        ...
        <div class="cp-calendar-month">
            <div class="cp-calendar-weekdays">
                <div class="cp-weekday"><?php esc_html_e('Sun', 'campaign-office'); ?></div>
                <div class="cp-weekday"><?php esc_html_e('Mon', 'campaign-office'); ?></div>
                <div class="cp-weekday"><?php esc_html_e('Tue', 'campaign-office'); ?></div>
                <div class="cp-weekday"><?php esc_html_e('Wed', 'campaign-office'); ?></div>
                <div class="cp-weekday"><?php esc_html_e('Thu', 'campaign-office'); ?></div>
                <div class="cp-weekday"><?php esc_html_e('Fri', 'campaign-office'); ?></div>
                <div class="cp-weekday"><?php esc_html_e('Sat', 'campaign-office'); ?></div>
            </div>
                            ...
                            <div class="cp-day-events">
                                ...
                                <?php if (count($day_events) > 3) : ?>
                                    <span class="cp-more-events">
                                        <?php printf(esc_html__('+ %d more', 'campaign-office'), count($day_events) - 3); ?>
                                    </span>
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
    ...
    private function render_week_view($date, $events) {
        ...
        <div class="cp-calendar-week">
            <p><?php esc_html_e('Week view: showing events for the current week', 'campaign-office'); ?></p>
    ...
    private function render_list_view($events) {
        if (empty($events)) {
            return '<p class="cp-no-events">' . esc_html__('No events found for this period.', 'campaign-office') . '</p>';
        }
    ...
    public function render_event_map($atts) {
        ...
        if (empty($lat) || empty($lng)) {
            return '<p class="cp-map-notice">' . esc_html__('Location coordinates not available for this event.', 'campaign-office') . '</p>';
        }
        ...
        <div class="cp-event-map-wrapper">
            ...
            <p class="cp-map-address">
                ...
                <a href="<?php echo esc_url($google_maps_url); ?>" target="_blank" rel="noopener">
                    <?php esc_html_e('Get Directions', 'campaign-office'); ?>
                </a>
            </p>
        </div>
        <?php
    ...
    public function append_ical_download($content) {
        ...
        $button = sprintf(
            ...
            esc_html__('Add to Calendar (iCal)', 'campaign-office')
        );
    ...
    public function render_admin_calendar_page() {
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Event Calendar', 'campaign-office'); ?></h1>
