...
class CP_Icons_Browser {
	...
	private function init_categories() {
		$this->icon_categories = array(
			'all' => array(
				'name' => __('All Icons', 'campaign-office'),
				'icons' => $this->get_all_icon_names(),
			),
			'campaign' => array(
				'name' => __('Campaign Essentials', 'campaign-office'),
			...
			'arrows' => array(
				'name' => __('Arrows & Navigation', 'campaign-office'),
			...
			'communication' => array(
				'name' => __('Communication', 'campaign-office'),
			...
			'ui' => array(
				'name' => __('User Interface', 'campaign-office'),
			...
			'files' => array(
				'name' => __('Files & Documents', 'campaign-office'),
			...
			'data' => array(
				'name' => __('Data & Charts', 'campaign-office'),
			...
			'people' => array(
				'name' => __('People & Users', 'campaign-office'),
	...
	public function add_admin_menu() {
		add_submenu_page(
			'themes.php',
			__('Heroicons Browser', 'campaign-office'),
			__('Heroicons', 'campaign-office'),
			'edit_theme_options',
			'heroicons-browser',
	...
	public function enqueue_assets($hook) {
		...
		wp_localize_script('cp-icons-browser', 'cpIconsBrowser', array(
			...
			'i18n' => array(
				'copied' => __('Copied to clipboard!', 'campaign-office'),
				'copyFailed' => __('Failed to copy', 'campaign-office'),
				'loading' => __('Loading icons...', 'campaign-office'),
			),
		));
	}
	...
	public function render_browser_page() {
		...
		<div class="wrap cp-icons-browser-page">
			<h1><?php _e('Heroicons Browser', 'campaign-office'); ?></h1>
			<p class="description">
				<?php _e('Browse and copy beautiful Heroicons for use in your campaign website. Click any icon to copy its function call.', 'campaign-office'); ?>
			</p>
			...
			<div class="cp-icons-browser-controls">
				<div class="cp-icons-search">
					<input
						...
						id="cp-icon-search"
						class="regular-text"
						placeholder="<?php esc_attr_e('Search icons...', 'campaign-office'); ?>"
					/>
				</div>

				<div class="cp-icons-filters">
					<label for="cp-icon-category"><?php _e('Category:', 'campaign-office'); ?></label>
					...
					<label for="cp-icon-style"><?php _e('Style:', 'campaign-office'); ?></label>
					<select id="cp-icon-style">
						<option value="outline"><?php _e('Outline', 'campaign-office'); ?></option>
						<option value="solid"><?php _e('Solid', 'campaign-office'); ?></option>
						<option value="mini"><?php _e('Mini (20px)', 'campaign-office'); ?></option>
						<option value="micro"><?php _e('Micro (16px)', 'campaign-office'); ?></option>
					</select>

					<label for="cp-icon-size-display"><?php _e('Display Size:', 'campaign-office'); ?></label>
					<select id="cp-icon-size-display">
						<option value="sm"><?php _e('Small', 'campaign-office'); ?></option>
						<option value="md" selected><?php _e('Medium', 'campaign-office'); ?></option>
						<option value="lg"><?php _e('Large', 'campaign-office'); ?></option>
						<option value="xl"><?php _e('Extra Large', 'campaign-office'); ?></option>
					</select>
				</div>
			</div>

			<div class="cp-icons-stats">
				<span class="cp-icons-count"><?php printf(__('%d icons available', 'campaign-office'), count($this->icon_categories['all']['icons'])); ?></span>
			</div>
			...
			<div class="cp-icons-usage">
				<h2><?php _e('Usage Examples', 'campaign-office'); ?></h2>
				<div class="cp-usage-examples">
					<div class="cp-usage-example">
						<h3><?php _e('In PHP Templates', 'campaign-office'); ?></h3>
						<code>&lt;?php echo campaignpress_get_heroicon('star', 'outline'); ?&gt;</code>
					</div>
					<div class="cp-usage-example">
						<h3><?php _e('With Custom Size', 'campaign-office'); ?></h3>
						<code>&lt;?php echo campaignpress_get_ui_icon('calendar', array('style' => 'solid')); ?&gt;</code>
					</div>
					<div class="cp-usage-example">
						<h3><?php _e('In Gutenberg', 'campaign-office'); ?></h3>
						<p><?php _e('Use the "Heroicon" block from the block inserter', 'campaign-office'); ?></p>
					</div>
				</div>
			</div>
		</div>
		...
		<div id="cp-icon-copied-notice" class="cp-copied-notice" style="display: none;">
			...
			<span class="cp-copied-text"><?php _e('Copied to clipboard!', 'campaign-office'); ?></span>
		</div>
		<?php
	}
	...
	public function ajax_get_icon_svg() {
		...
		if (empty($icon)) {
			wp_send_json_error(__('Icon name is required', 'campaign-office'));
		}

		$icon_svg = campaignpress_get_heroicon($icon, $style, array('aria-hidden' => 'true'));

		if (empty($icon_svg)) {
			wp_send_json_error(__('Icon not found', 'campaign-office'));
		}
